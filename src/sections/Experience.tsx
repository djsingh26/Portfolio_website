    ...
    <section id="experience" className="scroll-mt-24 px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="relative">
          <div className="absolute left-5 top-0 h-full w-px bg-ink-950/15 md:left-1/2 dark:bg-white/15" aria-hidden="true" />
                ...
                <article className={`relative grid gap-5 md:grid-cols-2 ${index % 2 === 0 ? '' : 'md:[&>*:first-child]:col-start-2'}`}>
                  <div className="pl-14 md:px-8">
                    ...
                    <div className="rounded-lg border border-ink-950/10 bg-white p-6 shadow-soft dark:border-white/10 dark:bg-white/10">
                      ...
                      <h3 className="mt-2 text-xl font-bold text-ink-950 dark:text-white">{item.role}</h3>
                      <p className="mt-1 text-sm font-semibold text-ink-950/60 dark:text-stone-300">{item.company}</p>
                      <ul className="mt-5 space-y-3">
                        {item.achievements.map((achievement) => (
                          <li key={achievement} className="flex gap-3 text-sm leading-6 text-ink-950/70 dark:text-stone-300">
